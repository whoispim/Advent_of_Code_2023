...
def next_steps(heatloss, pos, steps_in_line, d):
    ...
    poss_dir = []
    if steps_in_line >= 4:
        poss_dir.extend([(d+1)%4, (d+3)%4])
    if steps_in_line < 10:
        poss_dir.append(d)
    for new_d in poss_dir:
...
print(f'The minimum heat loss the lava can incur',
      f'in an ultra crucible is {heat_dict[goal_key]}.') # <1030 >1009 not 1087?
